...
const AiReport = () => {
    ...
    const handleConnect = async () => {
        ...
        try {
            // Step 1: Connect & validate MT5 credentials
            ...
            // Step 2: Fetch & save historical trades into the DB
            const syncRes = await api.post(`/users/${user.user_id}/fetch-mt5-trades`);
            const { saved = 0, total = 0 } = syncRes.data ?? {};

            // Step 3: Reload the trade list so the AI report can use new data immediately
            const tradesRes = await api.get(`/trades/user/${user.user_id}`);
            setRawTrades(tradesRes.data);

            setShowConnectModal(false);
            toast.success(`${selectedPlatform} Account Connected!`, {
                description: `${saved} new trades synced (${total} total found). Run the Deep Scan to analyse them.`,
            });
            ...
            const detail = err?.response?.data?.detail ?? "Connection failed. Check your credentials and server name.";
            toast.error("MT5 Connection Failed", { description: detail });
        } finally {
                        ...
                        <div className="relative mt-12 rounded-3xl overflow-hidden border border-border bg-card/80 backdrop-blur-3xl shadow-2xl">
                            ...
                            <div className="px-6 py-32 flex flex-col items-center justify-center text-center relative z-10">
                                ...
                                <h3 className="text-3xl font-black mb-4 tracking-tighter text-foreground">
                                    {isGenerating ? "Synthesizing Ledger History..." : (
                                        rawTrades.length === 0 ? "No Trading Data Found" : "Ready to confront your data?"
                                    )}
                                </h3>
                                <p className="text-muted-foreground max-w-lg text-base leading-relaxed">
                                    {isGenerating
                                        ...
                                        : (rawTrades.length === 0
                                            ? "Please connect your MT5 account using the button above to securely sync your trade history, or manually add trades in the dashboard."
