...
import { AiMarketAnalysis } from '@/components/calendar/AiMarketAnalysis';
...
export default function EconomicCalendar({ isNested = false }: EconomicCalendarProps) {
    ...
    const renderEventRow = (event: EconomicEvent) => {
        ...
        return (
            <div key={event._id} className="relative mb-3 group/wrapper">
                <div
                    onClick={() => setExpandedId(isExpanded ? null : event._id)}
                    className={cn(
                        "relative cursor-pointer group flex flex-col md:flex-row items-center gap-4 py-4 px-6 rounded-2xl border border-border/50 bg-card/80 hover:bg-card transition-all duration-300 shadow-sm z-10",
                        isExpanded && "rounded-b-none border-b-transparent shadow-none bg-card",
                        !isExpanded && isNextUp && "border-blue-500/30 bg-blue-500/[0.02] shadow-[0_0_20px_rgba(59,130,246,0.05)]",
                        !isExpanded && isLive && "border-amber-500/20 bg-amber-500/[0.01]"
                    )}
                >
                    {/* Badges */}
                    ...
                    {/* Chevron */}
                    <div className={cn(
                        "ml-4 transition-all duration-300 opacity-30 group-hover:opacity-100",
                        isExpanded ? "rotate-180 opacity-100 text-primary" : "text-muted-foreground"
                    )}>
                ...
                <AnimatePresence>
                    {isExpanded && (
                        ...
                        >
                            <div className="p-4 pt-1 bg-card/60 backdrop-blur-md border border-t-0 border-border/50 rounded-b-2xl shadow-lg -mt-2 pb-6">
                                <AiMarketAnalysis event={event} />
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
